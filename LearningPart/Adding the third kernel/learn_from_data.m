...
filename = 'ouput_PrecisionRecall.mat';
save(filename,'opt_Lapl','optPrec','optRec');

% % % %% Check the distribution of the eigenvalues
% % % lambda_sym = sort(lambda_sym);
% % % n_sec = 4;
% % % extension = (lambda_sym(length(lambda_sym)) - (lambda_sym(2)))/4; %extension of each section in terms of lambdas, not indices
% % % sections = zeros(1,n_sec);
% % % A = (lambda_sym(2)+extension);
% % % G = (lambda_sym <= A);
% % % section(1) = length(find(G));
% % % for i = 2:n_sec
% % %     A = (lambda_sym(2)+(i-1)*extension);
% % %     B = lambda_sym(2)+(i)*extension;
% % %     G = A <= lambda_sym <= B;
% % %     sections(i) = length(find(G)); %count the number of occurrences the find function returns
% % % end
% % % 
% % %  figure('Name','lambdas distribution')
% % %  plot(1:n_sec,sections)