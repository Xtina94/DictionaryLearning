...
for repetitions = 1:1
    ...
    switch flag
        ...
        case 2
            ...
            load originalKernels.mat
            originalAlpha = zeros(length(originalKernels{1}'), param.S);
            for i = 1:param.S
                originalAlpha(:,i) = originalKernels{i}';
            end
            load UberData.mat
            TestSignal = X(:,91:110);
            TrainSignal = X(:,1:90);
    ...
    filename = strcat(num2str(attempt_n));
    ...
    % such that we can compare it with the algo that learns the behavior of the
    % kernels at first
