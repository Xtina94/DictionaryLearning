...
function [gamma_coefficients, err] = coefficient_update_interior_point(Data,CoefMatrix,param,sdpsolver)
...
% =========================================================================
    ...
    m = length(beta_coefficients) - 1;
    q = sum(param.K)+S;
    %alpha_coefficients = sdpvar(q,1);
    K = max(param.K);
    %lambda_powers_beta = param.lambda_powers;

    %% Setting the gamma coefficient vector (since we impose to know the betas)
    gamma_coefficients = sdpvar((K - m + 1)*param.S,1);
    
    
    %% Defining the column vectors Pnm (for gamma vector)

    B1 = sparse(kron(eye(S),Lambda(:,1:K - m + 1)));
    B2 = kron(ones(1,S),Lambda(1:K - m + 1));
    ...
    F = ((B1*gamma_coefficients).*param.beta_vector <= c*ones(l1,1)) + ((-B1*gamma_coefficients).*param.beta_vector <= 0*ones(l1,1));
...
% % %     F = (B1*alpha_coefficients <= c*ones(l1,1)) + (-B1*alpha_coefficients <= 0*ones(l1,1));
    ...
    gamma_coefficients = double(gamma_coefficients);
