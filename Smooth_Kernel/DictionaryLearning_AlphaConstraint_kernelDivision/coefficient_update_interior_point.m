...
%% Low part
B1_low = sparse(kron(eye(floor(S/2)),Lambda));
B2_low = kron(ones(1,floor(S/2)),Lambda(1:param.percentage,:));
B3_low = sparse(kron(eye(floor(S/2)),Lambda(1:param.N-thresh,:)));

alpha_low = param.alpha_vector_low;

Phi_low = zeros((floor(S/2))*(K+1),1);
for i = 1 : N
         r = 0;
        for s_low = 1 : floor(S/2)
            for k = 0 : K
...
F_low = (B1_low*alpha_low <= c*ones(l1_low,1))...
    ...
    + (B2_low*alpha_low <= (c+(10*epsilon))*ones(l2_low,1))...
    + (-B2_low*alpha_low <= -(c-(10*epsilon))*ones(l2_low,1));
...
%% High part
B1_high = sparse(kron(eye(floor(S/2)),Lambda));
B2_high = kron(ones(1,floor(S/2)),Lambda(param.N-param.percentage+1:param.N,:));
B3_high = sparse(kron(eye(floor(S/2)),Lambda(thresh+1:param.N,:)));

alpha_high = param.alpha_vector_high;

Phi_high = zeros((floor(S/2))*(K+1),1);
for i = 1 : N
         r = 0;
