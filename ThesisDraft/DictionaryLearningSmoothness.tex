...
\begin{equation}
  ...
  \label{eq:sviluppo}
\end{equation}
Once we obtained it, we insert this knew information about the kernels structure in the optimization objective function, where now the alpha vector has a reduced number of unknowns that have to be found. With the problem set in this way we can thus analyse the new results we obtain. \cri{verifica se nei punti precendenti hai dato abbastanza spazio a questo concetto di smoothness e di come si comporta un grafo soggetto a questa struttura, se non è così ribidisci il fatto che ci interessa sapere altro sulla smth perchè così il grafo risulta più frammentato e in passi successivi separarlo ptrebbe essere un notevole vantaggio}
