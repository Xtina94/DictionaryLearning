...
close all
addpath(genpath(pwd))
%addpath(genpath('/Users/Hermina/Documents/Graph_SP/installations/yalmip'));
%addpath(genpath('/Users/Hermina/Documents/Graph_SP/installations'));
%addpath(genpath('/Users/Hermina/Documents/Graph_SP/graph_and_dict_learning_pro'));
...
for sparsity = 1:15
    ...
    for trying = 1:5
        ...
        x = OMP_non_normalized_atoms(learned_dictionary,param.y, param.T0);
        error(sparsity,trying) = app_error(param.y, learned_dictionary, x);
        disp(error(sparsity,trying));
    end
    %figure();
    %wgPlot(uber_graph+diag(ones(param.N,1)),[lon,lat],'vertexWeight',diag(ones(param.N,1)) );
    %g = plot_google_map('maptype','roadmap');
    %% geo graph errors
    [geoL,~,~]=create_Laplacian_isolated(geo);
    [learned_dictionary, param] = construct_temp_dict(geoL,param);
    x_geo = OMP_non_normalized_atoms(learned_dictionary,param.y, param.T0);
    geo_error(sparsity) = app_error(param.y, learned_dictionary, x_geo);
    disp(geo_error(sparsity));

    [geoL,~,~]=create_Laplacian_isolated(geo_gk);
    [learned_dictionary, param] = construct_temp_dict(geoL,param);
    x_geo_gk = OMP_non_normalized_atoms(learned_dictionary,param.y, param.T0);
    geo_gk_error(sparsity) = app_error(param.y, learned_dictionary, x_geo_gk);
    disp(geo_error(sparsity));
    disp('----------------');
end
save evening