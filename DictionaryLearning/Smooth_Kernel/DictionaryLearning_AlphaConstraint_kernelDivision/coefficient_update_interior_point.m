...
thresh = param.percentage+55;

%% Low part
if n_low_ker ~= 0
    B1_low = sparse(kron(eye(n_low_ker),Lambda));
    B2_low = kron(ones(1,n_low_ker),Lambda(1:param.percentage,:));
    B3_low = sparse(kron(eye(n_low_ker),Lambda(1:param.N-thresh,:)));
...
if n_high_ker ~= 0
    ...
    B2_high = kron(ones(1,n_high_ker),Lambda(param.N-param.percentage+1:param.N,:));
    B3_high = sparse(kron(eye(n_high_ker),Lambda(thresh+1:param.N,:)));
